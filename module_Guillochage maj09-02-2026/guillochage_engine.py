...
class GuillochageEngine:
    ...
    def calculate_geometry(self, layers_state, brut_data):
        ...
        for layer in layers_state:
            ...
            rot_global = float(global_params.get("rotation", 0.0))
            rad_global = math.radians(rot_global)
            
            sin_a = abs(math.sin(rad_global))
            cos_a = abs(math.cos(rad_global))

            # 1. HAUTEUR PROJETÉE (Couverture verticale)
            # Différenciation Rectangle vs Ovale pour éviter les erreurs de marge
            if is_circle:
                # Pour un ovale, la hauteur projetée exacte est quadratique
                projected_h = math.sqrt((brut_w * sin_a)**2 + (brut_h * cos_a)**2)
            else:
                # Pour un rectangle, c'est la somme des projections des côtés
                projected_h = brut_w * sin_a + brut_h * cos_a
            
            # 2. LARGEUR PROJETÉE (Calibrage Fréquence)
            if is_circle:
                projected_w = math.sqrt((brut_w * cos_a)**2 + (brut_h * sin_a)**2)
            else:
                projected_w = brut_w * cos_a + brut_h * sin_a
                
            if projected_w < 1.0: projected_w = 1.0

            marge_user = float(global_params.get("margin_in", 0.0))
            
            # Pas de facteur artificiel (1.0), on couvre exactement la hauteur calculée
            h_eff = projected_h - (marge_user * 2)
            
            # Pour éviter le problème des lignes tangentes invisibles sur les ovales (10 vs 12 lignes),
            # on triche d'un pixel (epsilon) pour s'assurer que la ligne mathématique 'mord' dans la forme.
            if is_circle:
                h_eff *= 0.999 
            
            min_y_axis = -h_eff / 2
            step = h_eff / (nb_total - 1) if nb_total > 1 else 0
            if nb_total == 1: min_y_axis = 0; step = 0

            for i, line_data in enumerate(lines_list):
                if line_data.get("is_deleted", False): continue
                if not line_data.get("is_active", True): continue

                current_params = global_params.copy()
                current_params.update(line_data.get("override", {}))
                
                y_base = min_y_axis + i * step
                
                current_params["line_index"] = i
                current_params["total_lines"] = nb_total
                current_params["y_base"] = y_base
                ...
                cr, sr = math.cos(rad_global), math.sin(rad_global)
                ...
                # Fréquence ajustée à la largeur réelle traversée
                cycles_per_mm = nb_cycles_user / projected_w
                ...
                for j in range(steps + 1):
                    ...
                    dist_from_center = tx 
                    angle_wave = dist_from_center * cycles_per_mm * 2 * math.pi
    ...
    def _clip_polyline(self, points, is_circle, w, h, r_corner):
        ...
        def is_inside(p):
            ...
            if is_circle:
                ...
                return False
            if not (-rx <= px <= rx and -ry <= py <= ry): return False
            if r_corner <= 0: return True
            ax, ay = abs(px), abs(py)
            cx = rx - r_corner; cy = ry - r_corner
            if ax > cx and ay > cy:
                dx = ax - cx; dy = ay - cy
        ...
        if prev_inside: current_segment.append(p_prev)
            
        for i in range(1, len(points)):
            p_curr = points[i]
            curr_inside = is_inside(p_curr)
            if prev_inside and curr_inside: current_segment.append(p_curr)
